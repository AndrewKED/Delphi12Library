...
{
 Andrew Spencer examples:

 Some DESTEST results : 0A202020544F2059 -> 1CAB5BC75CD5D7D4
                        51442D3830203037 -> 0EC117F76ECCE2B9
                        544F20594F555220 -> B6391DE0AC3D11B6
                        and back again
 using a TK of E6A12F079D15C437

 as follows:

  with TDoubleDWORD(i64Key) do begin
    R := $E6A12F07;   // First 4 bytes (MSB)
    L := $9D15C437;   // Last 4 bytes (LSB)
  end;
  FBlockCipher.Free;
  FBlockCipher := TDESCipher.Create(i64Key,SizeOf(i64Key));

  with TDoubleDWORD(i64Original) do begin
    R := $0A202020;    // First 4 bytes (MSB)
    L := $544F2059;    // Second 4 bytes (LSB)
  end;
  i64Encrypted := (FBlockCipher as T64BitBlockCipher).EncryptedBlock(i64Original);
}
