...
begin
  ...
  case fRetention of
    ...
    begin
      ...
      if ((newSize <= fMaxPoints) and
          (newSize <> Length(fFIFOPoints))) then
      begin
        // If changing to a new, valid size of FIFO.

        // Create a temporary, empty array for the retained points.
        SetLength(transfer, newSize);
        for m := 0 to newSize-1 do
        begin
          transfer[m].x := 0.0;
          transfer[m].y := 0.0;
        end; // for
        ...
        // Copy the data back (note that m will be the number of points to copy)
        n := 0;
        while (n < m) do
        begin
